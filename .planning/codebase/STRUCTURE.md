...
├── .planning/                    # GSD planning state (phases 01–08, codebase map, research)
...
├── freetoken-rocm-probe/         # Early ROCm probe tooling (tokio/zig probes)
...
- Key files: `.planning/STATE.md` (current status), `.planning/ROADMAP.md` (7-phase roadmap), `.planning/REQUIREMENTS.md` (REQ-WIN-07/REQ-PERF-07/REQ-STAT-07)
...
- Key files: `output/P8_VARIANT_COMPILE_PROPOSAL.md` (variant OBJECT proposal), `output/quilt_note.md` (2026-08-30 attestation), `output/race_note.md` (race harness + honest results), `output/deep-research/1000t-s-at-8k-gfx1100.md`, `output/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md`, `output/technical-synthesis-gfx1100-wmma-vs-dp4a.md`
...
- Implementation: `kernels/matmul_iq4xs/impl_gemm_wmma_stream.hip` via `#ifndef TILE_M/TILE_N/P/USE_XOR` guards + a per-variant OBJECT in `kernels/matmul_iq4xs/CMakeLists.txt` with `target_compile_definitions` (pattern: `impl_gemm_wmma_64x32_p4_xor.hip` include-wrapper per `output/P8_VARIANT_COMPILE_PROPOSAL.md`)
