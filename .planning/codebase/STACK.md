...
- **JavaScript (Node .mjs)** — `freetoken-rocm-probe/src/qstar.mjs` (q* policy projector, offline)
- **Zig 0.16** — bundled portable compiler in `freetoken-rocm-probe/tools/zig-x86_64-windows-0.16.0/` (compiler only, nothing installed system-wide)
...
- OpenCL.dll (runtime-loaded, no SDK) — `freetoken-rocm-probe/src/bench_bp.cpp` PCIe bandwidth probe on Windows
...
- `.wslconfig` — `[wsl2] memory=28GB` required (frozen via fingerprint `benchmarks/lib/fingerprint.py` hashes `/mnt/c/Users/arhan/.wslconfig`)
