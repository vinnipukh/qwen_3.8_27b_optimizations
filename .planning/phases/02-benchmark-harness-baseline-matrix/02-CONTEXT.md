...
- `.planning/ROADMAP.md` — Phase 2 success criteria, WSL2 risk notes, binding methodology rules (§Methodology Rules)
- `.planning/REQUIREMENTS.md` §Benchmarking — BENCH-01..04 authoritative text
- `.planning/reference/ROADMAP-original.md` — original methodology rules (binding, inherited)
- `.planning/phases/01-environment-validation-stock-baseline/01-CONTEXT.md` — carried decisions D-01..D-07 (root-only, frozen ROCm 7.2.1/Adrenalin pairing, binary archive policy, model logistics)
- `.planning/phases/01-environment-validation-stock-baseline/01-SUMMARY.md` — open items carried into this phase: elevated registry command pending owner; `.wslconfig` must join fingerprint; v0.2.0 lacks "offloaded N/M" line → verbose per-layer lines are residency evidence
- `.planning/research/deep-research/REPORT.md` — HIP-vs-Vulkan decode gap evidence; librocdxg#60 ROCr busy-spin fingerprint note; WSL#40732 crash pattern behind fail-fast policy
