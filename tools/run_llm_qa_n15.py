...
binary_path = "/root/llama-custom-07/bin/llama-cli"
prompt = "Explain the difference between DP4A and WMMA on AMD RDNA3 architectures in two concise paragraphs."
output_path = "/mnt/e/Projects/qwen_3.8_27b_optimizations/benchmarks/results/phase7/llm_qa_N15.json"
...
print("Running N=15 LLM QA benchmark (temp=0, -n 128)...", flush=True)
...
for i in range(1, 16):
    ...
    out = res.stdout + "\n" + res.stderr
    # Extract Prompt: X t/s | Generation: Y t/s
    ...
    print(f"Run {i:2d}/15: Prompt: {pp_tps:6.1f} t/s | Gen: {gen_tps:5.1f} t/s | Latency: {elapsed_ms:7.1f} ms", flush=True)
...
result = {
    ...
    "binary": "llama-custom-07/bin/llama-cli",
    "prompt": prompt,
    "n_predict": 128,
    "temperature": 0.0,
    "runs": 15,
...
print(f"Successfully generated {output_path} (N=15, avg tok/s={avg_gen_tps}, median={med_gen_tps})")
