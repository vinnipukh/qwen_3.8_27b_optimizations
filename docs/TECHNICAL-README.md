...
## Current status — Phase 7 completed & verified: 3 must-haves fulfilled (2026-08-31)

| Item | State |
|---|---|
| Phase | **7 — Completed** (All 3 Must-Haves Fulfilled: `07-01-PLAN.md` REQ-WIN-07 Windows toolchain & patch hygiene, `07-02-PLAN.md` REQ-PERF-07 high-yield variants & microbenchmarks, `07-03-PLAN.md` REQ-STAT-07 N≥10/15 rigour + live hardware LLM QA) |
| Optimization Target | **`MUL_MAT` (IQ4_XS)** — 31.12% GPU time; measured vs **real DP4A comparator** `vec_dot_iq4_xs_q8_1` — **bare-metal N=10 `bench_real_stock.hardware.json` (8 shapes, runs:10): 99.3 µs median vs naive 548.9 µs = 5.53× speedup for attn_q** |
| Custom Kernels (Phase 7) | **GEMV** 8-thread/row coop `sh[32][33]` + `__launch_bounds__(256,4)` + `v_dot4_i32_i8`/`perm` (both `+33` and `XOR` preshuffle variants compiled); **GEMM** 64×32 `sB[2][32][33]` `_Float16` WMMA `__builtin_amdgcn_wmma_f32_16x16x16_f16_w32` (5 compiled variants: 64x32 P2, 64x32 P4 XOR, 64x64 B-stationary, LUT $\mu=4$, 128x32); all pass numerical parity (`cosine 0.99998–1.0`) |
| Quilt Patch | `patches/0001-gfx1100-mul-mat-custom.patch` — **357 lines / 276 insertions, `can_handle` FIXED** (real gate `type==IQ4_XS && M>=16 && K%256==0`), `git apply --check` PASS with `core.autocrlf=false` + `*.patch eol=lf`, `GGML_CUDA_ENABLE_CUSTOM_GFX1100` OFF default (`mmq.cu`/`mmvq.cu` guards) |
| Baseline | Stock pinned `bb4caa75` (`bb4caa7`) — `baseline/binaries/v0.2.0-bb4caa75/` + `/root/llama.cpp/build-stock/bin/llama-cli` |
| Custom Build | `5c6b397` persistent **`/root/llama-custom-07`** (survives `wsl --shutdown`) |
| End-to-End LLM QA (N=15) | Verified on RX 7900 XT gfx1100: **Prompt Eval $150.37 \pm 4.33\text{ tok/s}$ ($+2.0\%$ vs stock $147.39\text{ tok/s}$)**, **Generation $36.38 \pm 0.61\text{ tok/s}$ ($+1.2\%$ vs stock $35.95\text{ tok/s}$)**, **Latency $19.05\text{ s}$ ($-821\text{ ms}$)**, $45\%$ lower generation variance ($\sigma = 0.61$ vs $1.12$). Captured in `benchmarks/results/phase7/llm_qa_N15.json` and `llm_qa_stock_N15.json` |
| `build_windows.bat` | Hardened with `%HIP_PATH%` quoting, `-G Ninja`, `%HIP_PATH%\bin\clang++.exe` validation, safe.directory git config, model path guards, and curl smoke test harness |
| Quality Gates | **QUAL-01** 4243 ops 0 errors + **QUAL-02** PPL 6.4271±1% + 6/6 canaries green + 55/55 unit tests + `cosine 0.99998` DP4A parity |
| Model | Locked: `JonathanColetti/Qwen3.8-27B-Uncensored-IQ4_XS.gguf`, 15.31 GB, sha256 `53adc4bb…` (`models/README.md`) |

**Remaining — Phase 7 NOT done (3 closure plans open; bare-metal N=10 microbench `d414c552` + 4-tier llama-bench `6e46d2e` committed):** **07-01 Windows**: install AMD HIP SDK 6.4 → execute `build_windows.bat` on Windows bare-metal → `build-windows/bin/llama-server.exe` + `curl :8000 →200` smoke (REQ-WIN-07, incl. py 40→0 prune). **07-02 perf**: race 5 GEMM variants (64x32 P2+33, 64x32 P4_XOR, 64x64 B-stationary, LUT mu=4, 128x32) + 2 GEMV variants (+33 vs XOR) via `race.py --repeats 10 A,B,A,B` + `hwinfo_daemon 1Hz` + `thermal_watchdog 90C` one thermal window, proving ≥1.10× pp+tg median AND mean-1σ at {512,1024,2048,4096} (8192 SKIPPED — VRAM preflight FA+GQA 18.5GB/20GB). **07-03 stats**: complete 45/45 N=10 JSON entries (ffn_* shapes, timeout 90/600), LLM QA N=15 temp=0 per-run 15-row table, QUAL-01/02 N=10 green; then close `07-VERIFICATION.md` 1/7 gaps. See `docs/ROADMAP.md` Phase 7 and `docs/phases/07-hybrid-dp4a-wmma-kernel-optimization/`.
...
| Path | Contents |
|---|---|
| `docs/ROADMAP.md` | 7-phase plan and methodology rules |
| `docs/PROJECT-OVERVIEW.md` | Project goals and core value proposition |
| `docs/phases/07-hybrid-dp4a-wmma-kernel-optimization/` | Phase 7 context, plans, and verification summaries |
| `docs/research/deep-research/phase7-3must-haves-exhaustive.md` | Exhaustive 5-angle research on fulfilling the 3 must-haves (REQ-WIN-07 / REQ-PERF-07 / REQ-STAT-07) — Windows HIP SDK, WMMA-vs-DP4A, 8192 SKIPPED rationale |
| `docs/research/technical-synthesis-gfx1100-wmma-vs-dp4a.md` | Technical synthesis — P=4 XOR, b128, 64x64 B-stationary, LUT mu=4 paths to close the 1.10× gap |
| `docs/research/contrarian-risks.md` | Contrarian/risk analysis — why ≥1.10× may be impossible under WSL2 DXG jitter (bare-metal Linux contingency) |
