...
4. **Cooperative Wave32 DP4A + Streaming WMMA (Phase 7 hybrid, re-scoped high-yield).** Decode (M=1): stock MMVQ single-warp-per-row (RDNA3 `calc_nwarps=1`) → 8-thread coop, 32 rows/block, `sh[32][33]` padded vs XOR preshuffle `x'=(y%(64/8))⊕x` (both variants raced); `ulong2`/`float4` `b128` `16B` coalesced, `__launch_bounds__(256,4)` → `≤64 VGPR`. Prefill (M≥512): stock DP4A on shader ALUs 512 ops/CU/clock → WMMA hardware cores 1024 ops/CU/clock (`__builtin_amdgcn_wmma_f32_16x16x16_f16_w32`, `_OPSEL` half-select, `wave32`), double-buffered `sB[2][32][33]` stride-33 (`P=2`) vs quad-buffered `sB[4][32][32]` XOR `P=4` (MARLIN), K_TILE=32, `sched_barrier 0x0080` (DS) / `0x0008` (WMMA), `B-stationary` `8 VGPR` frag (`v16f16`/`v8f32`), offline `16×64` swizzle, LUT `μ=4` (16-entry half, `32B/LUT`) vs inline `d*(ls-32)*kvalues`; `T=64 →64×` reuse per `output/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md` `gemm_optimization T=64`. Both templated `template<int WarpSize>` with `static_assert 32`. Files: `impl_gemv_dp4a_gfx1100.hip` (LDS `[32][33]` vs XOR), `impl_gemm_wmma_stream.hip` (64×32 P=2 vs 64×64 P=4 XOR), `impl_gemm_lut_iq4xs.hip` (μ=4 LUT), `tools/swizzle_iq4xs.py`, `amd_matrix_instruction_calculator` oracle, `bench --runs 10` (`N=10` `median/mean/stddev/p95`), vendored into `custom_gfx1100/*.cuh`.
...
6. **Phase 7 high-yield inner pipeline (re-scoped):** `Q8_1` quant (`amax/127` → `half2 ds`) → LDS `[2..4][32][33]` double-buffer `P=2` (`sB[2][32][33]` stride-33) / `P=4` (`sB[4][32][32]` XOR `x'=(y%(64/8))⊕x`) with `__builtin_amdgcn_sched_barrier(0x0080)` (DS) / `0x0008` (WMMA) pinning `GMEM→VGPR→LDS→VGPR→WMMA` 4-stage overlap → `B-stationary` weight frag `8 VGPR` (`v16f16`, `b_frag`) in VGPRs + activation streamed via LDS → `b128` `float4`/`ulong2` `16B` coalesced (`32 thr×4B→8×16B` via `__builtin_amdgcn_global_load_b128`, `SWDEV-556587`) + offline `16×64` swizzle to `128B` lines (`tools/swizzle_iq4xs.py`) → WMMA `__builtin_amdgcn_wmma_f32_16x16x16_f16_w32` (`_OPSEL` false for low half, `wave32` replicates `0–15→16–31`, `1024 ops/CU/clock`) — LDS banking `32×4B` 8-phase `ds_write_b128` (`0~7…56~63` conflict-free iff consecutive) and `ds_read_b128` `4-way→0` via `+33` (`+3%`) vs XOR `0%` per CK Tile `lds_bank_conflicts.html`; tiling `T=64 →64×` reuse (`loads/output = K·(1/M+1/N)`, `2K/T`) per CK Tile `gemm_optimization.html` (see `output/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md` LDS 32×4B 8-phase, `gemm_optimization T=64`). `amd_matrix_instruction_calculator` oracle validates `VGPR ≤64` (`A/B 8 VGPR fp16 / D 8 VGPR wave32`) before commit; `bench --runs 10` (`N=10` `median/mean/stddev/p95`) raced via `race.py --repeats 10` interleaved (`adelj88` pattern).
...
8. Run closes with `CHECKSUMS.sha256`; `publish_matrix.py` aggregates stock vs custom; `KERNEL-BENCH-DIFF.md` §8 and `PUBLICATION.md` §8 variant racing table (64×32 P2+33 vs 64×32 P4+XOR vs 64×64 P4+XOR vs LUT μ=4 vs W8A8 α=0.5, `N=10` `median/mean/stddev`) document the delta; gates assert QUAL-01 0 errors and QUAL-02 within 1% before verdict.
...
│   │                               #   impl_gemv_gfx1100.hip (naive-win) + impl_gemv_dp4a_gfx1100.hip (15,186 B, 07-02: LDS [32][33] vs XOR),
│   │                               #   impl_gemm_wmma.hip (naive-win) + impl_gemm_wmma_stream.hip (13,610 B, 07-03: 64×32 P=2 vs 64×64 P=4 XOR),
│   │                               #   impl_gemm_lut_iq4xs.hip (μ=4 LUT, 16-entry half) + tools/swizzle_iq4xs.py (16×64 swizzle),
│   │                               #   amd_matrix_instruction_calculator oracle + bench --runs 10 (median/mean/stddev/p95),
│   │                               #   test_*/bench_*.cpp (vs real-stock DP4A, N=10), CMakeLists.txt
