...
**Primary artifact:** `JonathanColetti/Qwen3.8-27B-Uncensored-IQ4_XS.gguf` (15.31 GB, sha256 53adc4bb…) — see `docs/research/MODEL-DECISION.md`
...
- [x] **KERN-01**: Kernel playground scaffold operating the full pipeline: CPU reference → HIP implementation → numerical comparison → microbenchmark (demo op `dequant_iq4_xs` only per owner lock D4-00-1; vendored `block_iq4_xs.h` D4-00-2; reuse `benchmarks/lib/store.py` D4-00-3; tight gate max_abs 1e-5/mean 1e-6/cosine 0.99999 +10× D4-00-4; templated `WARP_SIZE` D4-00-5 — see `docs/phases/04-kernel-playground-scaffold/04-CONTEXT.md`) — **DONE 2026-08-25**
...
### Phase 7 — Hybrid + Windows + 10% + 10× Rigour (added 2026-08-28, owner 3 wishes + deep-research report `docs/research/deep-research/1000t-s-at-8k-gfx1100.md`)
...
Feasibility + math: `docs/research/CONTEXT-SCALING.md`. Model natively supports 262,144 ctx; only 16/64 layers carry KV (~64 KiB/token f16); 48 DeltaNet layers hold constant state.
