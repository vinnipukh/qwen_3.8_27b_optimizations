...
Phase 7 must close 7 truths: 4 original (real DP4A comparator `N=10`, DP4A GEMV `>1.2× median` + `>38 t/s decode`, WMMA GEMM `>950 t/s prefill` with `64×32 [2][32][33]`, quilt OFF/ON + QUAL-01/02 green `N=10`) plus 3 owner must-haves added 2026-08-28 (Windows-native `≤2` langs gate, `≥1.10×` `pp+tg` at 5 tiers gate, `N=10`/`N=15` averaging gate). All prior E2E `808→849 pp4096 (+5.1%)` [VERIFIED: docs/STATE.md:stopped_at] is **insufficient** — the gate is `1.10× median` and `mean−1σ ≥1.10×` per tier per split [CITED: docs/REQUIREMENTS.md REQ-PERF-07].

The artifactual path is low-risk (kernels exist: `real_stock_dp4a_comparator.hip` 84 µs vs 543 µs [VERIFIED: docs/STATE.md], `impl_gemv_dp4a_gfx1100.hip` LDS `[32][33]` + `__launch_bounds__(256,4)` + `sudot4/perm` [VERIFIED: kernels/matmul_iq4xs/impl_gemv_dp4a_gfx1100.hip:1-80], `impl_gemm_wmma_stream.hip` `64×32 [2][32][33]` + `wmma_f32_16x16x16` [VERIFIED: kernels/matmul_iq4xs/impl_gemm_wmma_stream.hip:1-80]). The risk is **measurement** — WSL2 DXG adds `15–30 µs` jitter flattening `1.178→1.00` and lies about VRAM (`80 GiB` reported vs `3.48 GiB` contiguous fail) [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md], `rocprofv3` is Instinct-only/404 on WSL2 [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md], and `800 GB/s` caps naive `1000 tok/s` (`15.3 TB/s` needed) [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md].

High-yield levers that actually move the `≥1.10×` needle on 5-tier `N=10` are: scale tile `64×32→64×64` (64× reuse vs `2K` naive, `T=64` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]), replace `+33` (`+3%` LDS, `4-way→0` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]) with `XOR preshuffle x'=(y%(64/8))⊕x` 0-overhead for `64×64` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md], push pipeline `P=2→P=4` (`GMEM→VGPR→LDS→VGPR→WMMA` overlap, `sched_barrier 0x0080/0x0008` [CITED: llvm.org/docs/AMDGPUUsage.html]), `B-stationary` weight in VGPR + `float4`/`global_load_b128` `16B` coalescing (`32 thr×4B→8×16B` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]), offline `16×64` swizzle to `128B` cache lines + `LUT μ=4` 16-entry half bake (`d*(ls-32)` offline) [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md], and `SmoothQuant α=0.5` (`s_j=max|X_j|^α/max|W_j|^{1-α}` fused into `rmsnorm`, `W8A8` arm) [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md].

**Primary recommendation:** Ship Phase 7 as **two parallel tracks in one thermal window**: (A) WSL2 bare-metal `N=10` microbench + `N=10` `llama-bench` paired A/B across 5 tiers + `N=10` quality gates on `build-custom` (proves `≥1.10×` vs *real* DP4A); (B) Windows 11 HIP SDK native `build_windows.bat` (`HIP_PATH/bin/clang++.exe --offload-arch=gfx1100 -G Ninja`) `llama-server.exe :8000 →200` smoke + at least compile-gate + one `llama-bench` smoke tier to prove toolchain parity; interleave all benches via `race.py --repeats 10` pattern to kill thermal bias [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]; verify winner via `rocprof lds_bank_conflict 0` + `amd_matrix_instruction_calculator VGPR ≤64` + `llvm-objdump v_wmma/v_dot4` before declaring PASS.
...
| LDS banking & coalescing (`+33` vs `XOR`, `b128` `float4`) | GPU kernel (HIP, `ds_write_b128` 8-phase) | — | `32 banks×4B` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] is CU SRAM, not host |
...
| llama.cpp | pinned `bb4caa75` + quilt `patches/0001-gfx1100-mul-mat-custom.patch` (355 lines, OFF default) [VERIFIED: docs/STATE.md + patches/0001*] | Baseline + custom dispatch (`mmq.cu`/`mmvq.cu` `#if GGML_CUDA_ENABLE_CUSTOM_GFX1100`) | Anti-fork discipline; `git apply --check` bisectable |
...
| rocWMMA | **2.2.1** header-only `rocwmma/rocwmma.hpp` [CITED: rocm.docs.amd.com/projects/rocWMMA/] | Optional WMMA wrapper (alternative to raw `__builtin_amdgcn_wmma*`); header-only → no runtime | `≤2` langs gate: no Python/JS server; compiles with `clang++.exe --offload-arch=gfx1100` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] |
| amd_matrix_instruction_calculator | latest `ROCm/amd_matrix_instruction_calculator` (star 143) [CITED: github.com/ROCm/amd_matrix_instruction_calculator] | Pre-commit VGPR/layout oracle (`-a gfx1100 -i wmma_f32_16x16x16_f16 -d`, `--register-layout --csv`) | Predicts `A_frag 8 VGPR / D 8 VGPR wave32 → ≤64 VGPR` before code lands [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] |
| Composable Kernel (CK) Tile docs + `ck_tile/hardware/lds_bank_conflicts.html` | CK **1.2.0** [CITED: rocm.docs.amd.com/projects/composable_kernel/] | LDS banking (`32 banks×4B`, `ds_write_b128` 8-phase, `+33` vs `XOR` `x'=(y%(64/8))⊕x` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]) + GEMM tiling `T=64→64×` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] | Authoritative for `lds_bank_conflict` counter and tiling math |

### Supporting
| Library | Version | Purpose | When to Use |
|---------|---------|---------|-------------|
| `adelj88/rocm_wmma_gemm` (tune/race pattern) | 15★, 62 commits [CITED: github.com/adelj88/rocm_wmma_gemm] | Template for tile sweep (`tune.py` Genetic+RF surrogate `budget 100` + `race.py --repeats 10` interleaved) [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] | Ritual for `N=10` `REQ-STAT-07` interleaving to kill thermal bias; do not fork whole lib |
...
| `rocprofv3` (native bare metal only) | ROCm `6.4+` (Instinct-only matrix) [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] | `lds_bank_conflict 0` + `global_load_b128` coalescing count on winner variant | **WSL2 blind** — must run on native Windows/Linux bare metal, not under `librocdxg` [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] |
...
| `+33` padding `[32][33]` | `XOR preshuffle` 0-overhead | `+33` proven `+3%` (4224 vs 4096B) [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]; XOR saves `~1 KB` at `64×64` but adds `^` per access — race both |
| `P=2` double-buffer | `P=4` quad-buffer (`MARLIN` `P=4` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]) | `P=4` hides `GMEM→LDS` at `8192` but doubles LDS (`×2` vs `×4` buffers) — only use if `64×32` `4 KB` leaves headroom under `64 KB` CU |
| `IQ4_XS` primary | `SmoothQuant W8A8 INT8 WMMA` arm (`α=0.5` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]) | `W8` halves weight mem but costs `~2×` bits vs `4.25 bpw`; useful as comparator for `8192` `pp` if `≥1.10×` fails on `IQ4_XS` alone |
...
# Expect 200 + choices[0].message.content [CITED: docs/REQUIREMENTS.md REQ-WIN-07]

# Pre-commit oracle (offline, not shipped — satisfies ≤2 langs)
pip install tabulate typing_extensions  # calculator prereq [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]
...
void gemv_dp4a_coop_kernel(const block_iq4_xs* W, const float* x, float* y, int64_t K, int64_t N) {
  __shared__ float sh[32][33]; // +33 kills 32-bank conflict [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]
...
**What:** `64×32` per block (`4×2` warps, 256 thr) of `16×16×16` WMMA tiles (`__builtin_amdgcn_wmma_f32_16x16x16_f16_w32`, `wave32` replicates `0–15→16–31` [CITED: gpuopen.com/learn/wmma_on_rdna3]), double-buffered `sB[2][32][33]` (`_Float16`, `+3%` LDS, `ds_write_b128` 8-phase `0~7…56~63` conflict-free iff consecutive [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]) vs `XOR preshuffle x'=(y%(64/8))⊕x` 0-overhead [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]; `P=4` variant `sB[4][32][32]` (`MARLIN` `P=4` hides `GMEM→LDS` while WMMA runs [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]) pinned with `__builtin_amdgcn_sched_barrier(0x0080)` (DS) / `0x0008` (WMMA) [CITED: llvm.org/docs/AMDGPUUsage.html]; `B-stationary` (weight frag `A/B 8 VGPR fp16 / D 8 VGPR wave32` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] in registers, activation streamed via LDS) + `float4`/`global_load_b128` `16B` (`32 thr×4B→8×16B` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]) + offline `16×64` swizzle to `128B` cache lines [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]; dequant `d*(ls-32)*kvalues` overlapped with previous WMMA (pipelined), `LUT μ=4` 16-entry half bakes `d*(ls-32)` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]; fallback `TILE_M=16` when `M<512`.
**When to use:** `M≥128` prefill (pp), especially `M=8192` (`64×64` → `64×` reuse vs naive `2K` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]) where `800 GB/s` roof dominates.
**Example:**
```cpp
...
//              rocprof --metric lds_bank_conflict  # expect 0 on winner [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]
//              python matrix_calculator.py -a gfx1100 -i wmma_f32_16x16x16_f16 -d  # VGPR ≤64
```

### Anti-Patterns to Avoid
- **Microbench vs naive scalar only:** Banned for Phase 7 verdict; must be vs *real* `vec_dot_iq4_xs_q8_1` DP4A (`bench_real_stock` 84 µs baseline) — prior `6–7×` vs naive is not the gate [VERIFIED: docs/STATE.md + docs/REQUIREMENTS.md REQ-PERF-07].
- **Single-run `tok/s` claim:** Banned — every number is `median`+`mean`+`stddev`+`p95` over `N=10` (LLM QA `N=15` + per-run table) [CITED: docs/REQUIREMENTS.md REQ-STAT-07].
- **`cl` for `.hip` on Windows:** `MSVC cl` cannot compile HIP kernels; `HIP_PATH/bin/clang++.exe --offload-arch=gfx1100` is mandatory (see `build_windows.bat`) [VERIFIED: docs/STATE.md + 08-CONTEXT].
...
- **LDS without banking fix:** `sB[32][32]` naive `float16` causes `4-way conflict → -75% BW` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]; always `+33` or `XOR`.
- **Scalar `31×` `4B` loads for `qs`:** Flatten to `ulong2`/`float4` `16B` `b128` coalesced; `32 thr×4B→8×16B` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md].
...
| Global coalescing width | Scalar `uint8` loads | `__builtin_amdgcn_global_load_b128`/`float4` `16B` + `__builtin_assume_aligned(ptr,16)` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] | `SWDEV-556587` vector path is `8×` fewer transactions; scalar stays `dword` bound |
| GEMM tiling reuse math | Ad-hoc `BLOCK_M` | `loads/out = K·(1/M+1/N)` → `T=64→64×` reduction [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] | Naive `16×16` `512K` loads vs ideal `32K` (`16×`) — tiling is the `800 GB/s` roof fix |
| Pipelining overlap | `__syncthreads()` only | `GMEM→VGPR→LDS→VGPR→WMMA` 4-stage + `sched_barrier` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md + llvm.org/docs/AMDGPUUsage.html] | `__syncthreads` alone does not hide `GMEM` latency while WMMA runs (`MARLIN P=4` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]) |
| Activation smoothing for `W8A8` | Per-channel quant hack | `SmoothQuant s_j = max|X_j|^α / max|W_j|^{1-α}`, `α=0.5` fused into `rmsnorm` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md] | `α=0.5` migrates `70→1` outlier without GEMM-incompatible per-channel scales |
| Dequant elision | Inline `kvalues` multiply per WMMA | `LUT-GEMM μ=4` 16-entry half LUT per tile (bake `d*(ls-32)`) [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md] | Saves `scale*(ls-32)` multiply in hot loop; `32B/LUT` fits `sB` spare |
...
**What goes wrong:** `bench_gemv_dp4a` vs `stock_hip_comparator.hip` (543 µs) reports `6.43×` and declares win, but `bench_real_stock` shows real DP4A is `84 µs` [VERIFIED: docs/STATE.md]; `>1.2×` vs naive becomes `1.00×` vs DP4A under jitter [VERIFIED: docs/STATE.md].
...
**Warning signs:** `BASELINE_DP4A.md` `speedup 3.89–13.81×` [VERIFIED: docs/STATE.md] appears only when comparator is linked.

### Pitfall 2: WSL2 DXG Lies About VRAM and Jitter — `1.10×` Vanishes
**What goes wrong:** `llama-bench p=8192` appears `+5.1%` (`808→849`) on WSL2 but `mean−1σ` dips below `1.10×`; `800 GiB` reported vs `3.48 GiB` contiguous fail [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] causes `SKIPPED` without pre-flight; `15–30 µs` DXG tax flattens `1.178→1.00` [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md]; `3–5 OOMs → BSOD` [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md].
**Why it happens:** `WSL2` `DXGI/Hyper-V` `16 GiB` invisible overhead + non-realtime VM [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md]; `HSA_ENABLE_DXG_DETECTION=1` is shim.
**How to avoid:** VRAM pre-flight: require `>2 GB` free + `KV≈128 KiB/tok ×8192≈1 GB` budget [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] or mark `8192: SKIPPED (preflight)`; run `N=10` interleaved in one thermal window with `hwinfo_daemon 1Hz` + `thermal_watchdog 90C`; step-up `-ngl 0→10→99` before `p=8192`.
...
**What goes wrong:** `sB[32][32] _Float16` naive `64 KB` per `64×64` tile hits `4-way` conflict on `ds_read_b128` vertical `0:3+20:23` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] → `1.00×` even with WMMA.
**Why it happens:** `bank=(addr/4)%32`, `32 banks×4B` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]; `read_b128` needs `×` not `+` fix.
**How to avoid:** Variant A `+33` (`[32][33]` `+3%` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]) vs Variant B `XOR preshuffle x'=(y%(64/8))⊕x` `0%` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]; verify `rocprof lds_bank_conflict ==0` on bare metal (WSL2 blind) [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md].
...
**Why it happens:** `MARLIN P=4` needed for `K=64` at `batch 64` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]; `GMEM→VGPR` is slowest stage [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md].
...
**Why it happens:** Only `HIP SDK clang++.exe` understands AMDGPU builtins; `cl` is MSVC [VERIFIED: docs/STATE.md 07-04 gaps].
...
**Why it happens:** REQ-STAT-07 mandates `N=15` fixed prompt `temp=0` + per-run table [CITED: docs/REQUIREMENTS.md REQ-STAT-07].
...
```batch
REM Source: docs/REQUIREMENTS.md REQ-WIN-07 + 08-CONTEXT
set HIP_PATH=C:\Program Files\AMD\ROCm\6.4
...
|--------------|------------------|--------------|--------|
| Naive float `stock_hip_comparator.hip` 543 µs (scalar dequant) | Real DP4A `vec_dot_iq4_xs_q8_1` 84 µs (`sudot4`+`perm`×6, `ls-32`, `d*low2float`) [VERIFIED: docs/STATE.md] | Phase 7 07-01 (Aug 2026) | Comparator `6.43×` faster baseline — gate is `>1.2×` vs 84 µs, not 543 µs |
...
| `P=2` double-buffer `GMEM→LDS` | `P=4` quad-buffer `GMEM→VGPR→LDS→VGPR→WMMA` + `sched_barrier 0x0080/0x0008` [CITED: llvm.org/docs/AMDGPUUsage.html + docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md] | MARLIN 2023, CK `gemm_optimization.html` | Hides `800 GB/s` stall at `8192`; required for `mean−1σ ≥1.10×` |
| Scalar `4B` global loads (`dword`) | `b128` `float4`/`ulong2` `16B` (`global_load_b128`, `32 thr→8×16B`) [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] | `SWDEV-556587` | `8×` fewer transactions; offloads `800 GB/s` roof |
| Inline `d*(ls-32)*kvalues` per WMMA | `LUT-GEMM μ=4` 16-entry half LUT (`d*(ls-32)` baked) [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md] | ICLR 2024 | `~5%` hot-loop saving; `32B/LUT` fits spare LDS |
| Per-token Q8_1 only (`max 70` outliers [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]) | `SmoothQuant α=0.5` `s_j=max|X_j|^α/max|W_j|^{1-α}` fused into `rmsnorm` → `W8A8 INT8 WMMA` arm | ICML 2023 | `1.5×` paper → `1.2×` realistic; comparator arm if `IQ4_XS` alone misses `1.10×` at `8192` |
...
- `hipBLASLt` on `gfx1100` — excluded per ROADMAP (Tensile gap, `gfx1100` not in `hipBLASLt`); pure WMMA without `hipBLASLt` validated by CK docs [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md].

## Assumptions Log

| # | Claim | Section | Risk if Wrong |
|---|-------|---------|---------------|
| A1 | Stock DP4A at `84 µs` (`attn_q`) vs `543 µs` naive is representative across 8 Qwen shapes (range `3.89–13.81×` per BASELINE_DP4A.md) [CITED: docs/STATE.md] | Standard Stack / Pitfalls | Low — re-measure `bench_real_stock --runs 10` on bare metal; if shape spread differs, `>1.2×` gate still holds per-shape |
...
| A3 | `LDS 32 banks×4B`, `ds_write_b128` 8-phase `0~7…56~63` conflict-free iff consecutive, `ds_read_b128` needs `+33` or `XOR` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] | Architecture Patterns | Medium — if banking is `64×2B` variant, `+33` still works but XOR offset shifts; `rocprof lds_bank_conflict 0` resolves |
| A4 | `800 GB/s` (XT) / `960 GB/s` (XTX) pin caps `1000 tok/s` naive (`15.3 TB/s` needed) → `500–800 tok/s` fused ceiling [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] | Summary | Low — number is Wikipedia spec-derived; even if `1 TB/s`, conclusion unchanged (tiling required) |
| A5 | `WSL2` `15–30 µs` DXG jitter, `16 GiB` invisible overhead, `80 GiB` vs `3.48 GiB` fail, `BSOD` after `3–5 OOMs`, `rocprofv3` blind on WSL2 [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] | Summary / Pitfalls | Low — already observed (`avg 1.00 pk 1.178` [VERIFIED: docs/STATE.md]); mitigated by native `rocprof` + Windows path |
| A6 | `MARLIN P=4` sufficient to hide `GMEM→LDS` while WMMA runs at `8192` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md] | State of the Art | Medium — if `P=4` spills LDS beyond `64 KB` CU, `P=3` or `64×32` fallback is the mitigant; race picks winner |
| A7 | `LTR`? `SmoothQuant α=0.5` (or `0.75` for outlier-heavy) fused into `rmsnorm` yields `W8A8` `≈FP16` on `PPL 6.4271` gate [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md] | State of the Art | Medium — requires `512`-sentence Pile calibration offline; if `PPL` drifts `>1%`, `α` sweep `0.5/0.6/0.75` is the fix |
...
| A9 | `KV≈128 KiB/tok` GQA estimate → `8k≈1 GB` (`64L/8KV/128hd` `262 KiB/tok` worst) [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] | Summary | Medium — dump `llama-gguf --json` to replace estimate; if `KV` is `512 KiB/tok`, `8192` is OOM and `SKIPPED` rationale strengthens |
| A10 | `adelj88 tune.py` RF surrogate `budget 100` translates to our `64×32 vs 64×64 vs 128×32` sweep with `N=10` `median` picker [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md] | Standard Stack | Low — `budget 100` is illustrative; actual budget is `N=10` per variant `×6` variants = 60 compiles, tractable |
...
1. **VRAM pre-flight for `8192` on 20 GB**
   - What we know: `15.31 GB` model + `KV 1–2 GB` (`128 KiB/tok`) + `0.3 GB` transient = `16.7–18.9 GB` on `20 GB` [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md]; `800 GiB` lie risk [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md].
   - What's unclear: Measured `mem_get_info` free vs contiguous `3.48 GiB` fail [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] threshold on this `7900 XT` AD + VRAM? Exact `llama.cpp` `KV` per token for this `IQ4_XS` gguf.
...
3. **`P=2 vs P=4` occupancy tradeoff**
   - What we know: `P=4` is `GMEM→VGPR→LDS→VGPR→WMMA` 4-stage overlap [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]; `MARLIN P=4` sufficient [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md].
...
4. **`LUT μ=4` vs `μ=8` for `IQ4_XS`**
   - What we know: `μ=4` → `16` entries `32B/LUT` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]; `μ=8` → `256` `512B/LUT` still LDS-friendly but `4×` less reuse.
...
5. **`SmoothQuant W8A8` arm priority**
   - What we know: `O3` `1.56×` paper [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md]; `α=0.5` `s_j` [CITED: docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md].
...
6. **`b128` coalescing count verification**
   - What we know: `global_load_b128` exists (`SWDEV-556587`) [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]; `float4 16B` maps to it.
...
| `llama.cpp` pin `bb4caa75` + `build-stock`/`build-custom` | Paired `llama-bench` `N=10` 5 tiers | ✓ (tree present) | `bb4caa75` [VERIFIED: docs/STATE.md] | — |
...
| `rocprofv3` + `llvm-objdump` + `hipcc --save-temps` | `lds_bank_conflict 0`, `v_wmma/v_dot4`, `VGPR ≤64` gates | `llvm-objdump` ✓; `rocprofv3` ✗ on WSL2 (Instinct-only/404) [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] | LLVM `22.0.0git` | Run `rocprof` on native bare metal (Windows/Linux), not under `librocdxg`; `llvm-objdump` works everywhere |
...
| INTEG-02 (patch + gates) | Quilt `patches/0001-gfx1100-mul-mat-custom.patch` `git apply --check` PASS both OS + `QUAL-01 0 errors N=10` + `QUAL-02 PPL 6.4271 N=10` | integration | `git -C llama.cpp apply --check ../patches/0001*` && `cmake -B build-custom -DGGML_CUDA_ENABLE_CUSTOM_GFX1100=ON` && `run_op_gate.py --runs 10` && `run_model_gate.py --runs 10` | ✅ Patch 355 lines PASS [VERIFIED: docs/STATE.md]; ❌ `op_gate_custom N=10` not yet executed on metal (verifier GAP3) |
...
- [ ] `kernels/matmul_iq4xs/bench_* --runs 10 --json` re-bench on WSL2 bare metal (median/mean/stddev/p95 + speedup_median per 8 shapes) — `bench_real_stock` denominator 84µs [VERIFIED: docs/STATE.md]
...
- [ ] `rocprof lds_bank_conflict 0` on native bare metal for winning `XOR` vs `+33` variant (WSL2 blind [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md])
...
| V5 Input Validation | yes | `GGUF` `sha256 53adc4bb…` verified before bench; `K,N,M` bounds-checked (`m1 = min(m0+TILE, M)`), `hipMalloc` fail-fast, no `retry` loops (BSOD avoidance [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md]) |
...
| VRAM overcommit → host BSOD | Denial of Service | Pre-flight `>2 GB` free + `hipMalloc` probe + `RSS` guard `22.7 GB` fail threshold [CITED: benchmarks/RUNBOOK.md §thresholds]; no retry loops per microsoft/WSL#40732 [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md] |
...
| Uninitialized LDS read | Information Disclosure | `sB[2][32][33]` double-buffer + `__syncthreads()` uniform, prior `WMMA` uninit-LDS fix [VERIFIED: docs/STATE.md Phase 6] |
| Barrier divergence | DoS (hang) | Uniform `__syncthreads()` per `WARP_SIZE` templated, thermos remediation [VERIFIED: docs/STATE.md] |
...
- `gpuopen.com/learn/wmma_on_rdna3/` — RDNA3 WMMA `16×16×16`, `wave32` replication `0–15→16–31`, `512 FLOP/clock/CU` [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md]
- `rocm.docs.amd.com/projects/rocWMMA/` — header-only, `CUDA WMMA`-compat, `MFMA` vs `WMMA/SWMMAC` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]
- `rocm.docs.amd.com/projects/composable_kernel/.../lds_bank_conflicts.html` — `32 banks×4B`, `ds_write_b128` 8-phase, `+33 4-way→0` (`-75%` if not), `XOR` `x'=(y%(64/8))⊕x` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]
- `rocm.docs.amd.com/projects/composable_kernel/.../gemm_optimization.html` — `loads/out=K·(1/M+1/N)`, `T=64→64×`, `B-stationary`, 4-stage pipeline `GMEM→VGPR→LDS→VGPR→WMMA` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]
- `llvm.org/docs/AMDGPUUsage.html` — `sched_barrier 0x0080` DS, `0x0008` WMMA/MFMA, `sched_group_barrier` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]
- `rocm.docs.amd.com/projects/HIP/en/latest/how-to/hip_cpp_language_extensions.html` — `__launch_bounds__(256,4)`, `amdgpu_flat_work_group_size(256,256)`, `dim3` [CITED: docs/research/deep-research/1000t-s-at-8k-gfx1100.md]
- `docs/REQUIREMENTS.md` REQ-WIN-07 / REQ-PERF-07 / REQ-STAT-07 / BENCH-01 amended — `≤2` langs, `≥1.10× pp+tg` `mean−1σ`, `N=10`/`N=15` [CITED: local]
- `docs/STATE.md` `stopped_at` — `84µs vs 543µs`, `1.178 pk avg 1.00`, `WMMA [2][32][33]`, `355 lines` [CITED: local]
- `github.com/ROCm/amd_matrix_instruction_calculator` + `github.com/adelj88/rocm_wmma_gemm` — calculator `VGPR ≤64` + `tune.py budget 100` + `race.py --repeats 10` [CITED: docs/research/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md]
- `docs/research/deep-research/1000t-s-at-8k-gfx1100.md` — `800GB/s roof`, `KV≈128KiB/tok`, `WSL2 16GiB lie+BSOD`, `jitter`, `rocprof blind` [CITED: local]
- `docs/research/deep-research/custom_kernel_pdfs/SYNTHESIS.md` — `MARLIN P=4+16×64`, `LUT μ=4`, `SmoothQuant α=0.5` [CITED: local]
