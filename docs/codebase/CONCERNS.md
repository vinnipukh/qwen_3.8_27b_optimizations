...
- Issue: `find . -name "*.py" ! -path "./llama.cpp/*"` returns **40** and `*.mjs` returns **1** (`docs/research/freetoken-probe/src/qstar.mjs`). REQ-WIN-07 requires `==0`. Explicitly deferred to Phase 8 (`08-refactor-windows-native/`, not started).
- Files: `benchmarks/` harness, `benchmarks/results/phase7/race.py`, `tools/swizzle_iq4xs.py`, `tools/{ask_model,dump_gguf_fixtures,dump_matmul_fixtures}.py`, `benchmarks/bin/{run_op_gate,run_model_gate}.py`, `output/` python, `docs/research/freetoken-probe/src/qstar.mjs`
...
- Issue: `impl_gemv_dp4a_gfx1100-hip-amdgcn-amd-amdhsa-gfx1100.out` (11 KB) is **not** covered by `.gitignore` (only `*.out.resolution.txt` is); `kernels/matmul_iq4xs/impl_gemm_wmma_stream.hip.bak` is a stray backup in-tree; root-level `bench_*.bare.json` (4 files), `.playwright-cli/`, `temp_readme.txt`, `tools/__pycache__/` are untracked; `docs/research/freetoken-probe/tools/` vendor is 474 MB on disk (gitignored).
...
- Risk: `docs/research/freetoken-probe/tools/` vendors a zig 0.16.0 toolchain (~474 MB) in the working tree (gitignored). No pinned checksum recorded.
- Files: `docs/research/freetoken-probe/tools/`
