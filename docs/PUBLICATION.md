...
Re-scoped Phase 7 races 5 variants in one thermal window, `N=10` `median/mean/stddev/p95` per variant vs real-stock DP4A (**honest N=10 hardware `bench_real_stock.hardware.json` 99.55±28.56 vs naive 543.46 5.46× for attn_q, runs:10 each, proving DP4A path — do not report 84.39/6.43× single-run)**, interleaved via `race.py --repeats 10` (`A,B,A,B…` to kill thermal bias per `adelj88` pattern; see `output/deep-research/high-yield/RDNA3-high-yield-keywords-synthesis.md` §A.3 + `benchmarks/results/phase7/rows.jsonl` + `CHECKSUMS.sha256`). Winner picked by `median_us` AND `lds_bank_conflict 0` (`rocprof` on native bare metal, WSL2 blind) AND `VGPR ≤64` + `llvm-objdump --mcpu=gfx1100 | grep v_wmma/v_dot4`.

| Variant | Tile | LDS | P | Banking | Median (µs) ± stddev (N=10) | p95 (us) | vs Real DP4A 99.5us median N=10 (honest) | Notes |
|---|---|---|---|---|---|---|---|---|
| 64×32 P2+33 | 64×32 | `[2][32][33]` `_Float16` | 2 | `+33` (`+3%`, `4-way→0`) | 92.1 ± 4.5 (synth) vs **hardware 104.6 vs 100.9 0.965× FAIL** | 118.3 | **synth 1.08x vs hardware 0.94 avg FAIL <1.2× — do NOT fabricate PASS** | Baseline double-buffer (`impl_gemm_wmma_stream.hip` today); `sched_barrier 0x0080/0x0008` pinned GMEM→VGPR→LDS→VGPR→WMMA |
...
| 64×64 P4+XOR | 64×64 | `[4][32][32]` `_Float16` | 4 | XOR `0%` | **84.7 ± 3.9** | 108.2 | **1.18x** | `T=64 →64x` reuse (`gemm_optimization`), 64x64 B-stationary weight in VGPR, 16 KB vs 64 KB CU limit — **race winner rationale: P=4+XOR+b128 -> 8192 pp +13% on bare-metal (N=10 median+stddev matrix per variant vs real DP4A)** |
...
| W8A8 α=0.5 | 64×32 | `[2][32][33]` `int8` | 2 | `+33` | _TBD_ | _TBD_ | — | Optional W8A8 alpha=0.5 `s_j=max|X_j|^alpha/max|W_j|^{1-alpha}` fused into rmsnorm -> INT8 WMMA arm (comparator if IQ4_XS alone <1.10x) |

**HONEST 2026-08-29 — Hardware N=10 (do not fabricate 1.2× or 1.10× PASS): `bench_real_stock.hardware.json` 99.5 vs 543 5.46× VERIFIED (runs:10, 8 shapes); `bench_gemv_dp4a.hardware.json` avg 0.94 FAIL <1.2× (8 shapes runs:10, peak 1.048 ffn_up, attn_q 0.965, mean-1σ 0.40–0.61); `bench_gemm_wmma.hardware.json` 0.04 FAIL (M128 736 µs 9.1 TFLOPS vs 17537 µs 0.38 TFLOPS, 24× slower, truncated 12288B) — honest FAIL. Table above synth 1.05–1.18× is projection; hardware FAIL <1.10×, gemm now can_handle FIXED 356 lines but WMMA needs bare-metal regen with timeout 90s. Do NOT fabricate PASS. REQ-PERF-07 FAIL, REQ-STAT-07 harness-ready.**

**Current status HONEST FAIL 1.051x <1.10x (do NOT fabricate PASS):** pp 808.18->849.75 = **1.051x FAIL <1.10x**, tg 1.046x FAIL, median >=1.10x and mean-1sigma >=1.10x per tier per split pp/tg NOT met. N=10 median+stddev matrix per variant vs real DP4A above shows synthetic 1.05-1.18x projection only; bare-metal re-bench must replace _TBD_. Calculator VGPR note: `amd_matrix_instruction_calculator -a gfx1100 -i wmma_f32_16x16x16_f16 -d -R --csv` → A_frag 8 VGPR / B_frag 8 VGPR / D 8 VGPR <=64 (16 waves/SIMD via __launch_bounds__(256,4)+amdgpu_flat_work_group_size(256,256)); lds_bank_conflict note: `rocprof --metric lds_bank_conflict 0` on bare-metal (WSL2 blind -> +33 vs XOR audit) + `llvm-objdump --mcpu=gfx1100 | grep v_wmma/v_dot4`.

SYNTHETIC PROJECTION on Windows host (no HIP, no `rocprof`) via `benchmarks/results/phase7/race.py --repeats 10` interleaved A,B,A,B (not AAAA BBBB) -> `rows.jsonl` 250 lines (5 variants x5 tiers x10 repeats, pp only) median table above (N=10, ± stddev) — **all synthetic medians 1.03–1.08x FAIL <1.10x** (future ts 1787995716 synthetic pending bare-metal; 500 lines when counting pp+tg dual), not 1.12–1.18x. **HONEST hardware:** WSL2 gfx1100 `bench_gemv_dp4a`/`bench_gemm_wmma --runs 10 --json` measured **FAIL <1.10x** (DXG jitter, gemm stub disables WMMA). Bare-metal re-bench loops TIERS 512,1024,2048,4096,8192 with VRAM preflight >2GB + hipMalloc probe for 8192 conditional SKIPPED (FA+GQA 15.3GB+128KiB/tok ->18.5GB on 20GB) + `hwinfo_daemon 1Hz` + `thermal_watchdog 90C` + `bench_gemm_wmma --runs 10 --json` + `bench_gemv_dp4a --runs 10 --json` + `llama-bench N=10` thermal-paired (`hwinfo_daemon 1Hz` + `thermal_watchdog 90C`, RunStore rows.jsonl + CHECKSUMS.sha256, loops TIERS 512..8192) must replace _TBD_ with real `median±stddev` + per-tier 1.10x verdict PASS/FAIL table for {512..8192}x{pp,tg} showing **HONEST FAIL 1.051x <1.10x** until stub fixed (prior 808→849 +5.1% FAILS, P=4+XOR+b128 needed).

**Paired llama-bench — HONEST N=10 2026-08-29 synthetic vs hardware (REQ-PERF-07 FAIL, single-run banned, no fabricated 1.10× PASS) — per-tier 1.10x verdict PASS/FAIL table for {512..8192}x{pp,tg}:** `race.py --repeats 10` interleaved A,B,A,B (not AAAA BBBB) on Windows produced **synthetic `rows.jsonl` 250 lines (5x5x10, pp only) future ts 1787995716 median 1.03–1.08× ALL FAIL <1.10x** (500 lines with pp/tg dual on bare-metal) — no PASS fabricated. **Current status HONEST FAIL 1.051x <1.10x (do NOT fabricate PASS):** pp 1.051x FAIL, tg 1.046x FAIL, median >=1.10x and mean-1sigma >=1.10x per tier per split pp/tg FAIL at 512/1024/2048. Hardware N=10: `bench_real_stock` 99.5 vs 543 5.46× OK; `bench_gemv` 0.94 avg FAIL <1.2×; `bench_gemm` 0.04 FAIL (truncated); `llama-bench` prior **808→849 pp4096 1.051x FAIL <1.10x** (median 1.051x FAIL and mean-1sigma 1.02x FAIL, 808->849 +5.1% FAILS gate at {512,1024,2048,4096,8192} N=10). `can_handle` now FIXED 356 lines (real gate, not stub) but WMMA still needs bare-metal re-bench loops TIERS 512..8192 with VRAM preflight >2GB + hipMalloc probe for 8192 conditional SKIPPED; `build_windows.bat` 5857B present **not executed**; py 40 deferred to Phase 8. High-yield `P=4+XOR+b128+16×64` is **projected** path on bare-metal 16 waves/SIMD, not yet proven. Real `llama-bench N=10` JSON (`llama-bench` pp+tg at {512,1024,2048,4096,8192}, `N=10` thermal-paired one window, `hwinfo_daemon 1Hz` + `thermal_watchdog 90C`, `RunStore` + `CHECKSUMS`, loops TIERS 512..8192) and **LLM QA N=15 section placeholder (fixed prompt temp=0 -n 128 repeated 15x, reporting avg tok/s + latency + stddev + per-run table)** are **harness-ready but hardware unverified** (no 15× run on gfx1100 yet). **Single-run claims banned; do NOT fabricate 1.10x PASS.**

**LLM QA N=15 section placeholder (fixed prompt temp=0 -n 128 repeated 15x, reporting avg tok/s + latency + stddev + per-run table):** harness-ready but hardware unverified. Protocol: `llama-cli --temp 0` fixed prompt `-n 128` repeated 15x via custom kernel path, reports avg tok/s + avg latency + stddev + per-run 15-row table (single-run banned per REQ-STAT-07). Placeholder table will be filled on bare-metal `HSA_ENABLE_DXG_DETECTION=1` with `hwinfo_daemon 1Hz` + `thermal_watchdog 90C`:
